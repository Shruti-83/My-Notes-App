...
e.preventDefault();
try{
const {data} =await axios.post(backenedUrl + '/api/auth/send-reset-otp', {email})
...
toast.error(error.message)
  ...
  const onSubmitNewPassword = async (e)=>{
    ...
    try{
      const { data } = await axios.post(
        backenedUrl + '/api/auth/reset-password',
  ...
  return (
    <div className='flex items-center justify-center min-h-screen  bg-gradient-to-br from-blue-200 to-purple-400'>
       <img onClick={()=>navigate('/')}  className='absolute left-5 sm:left-20 top-5 w-28 m:w-32 cursor-pointer' src={assets.logo} alt="" />
    ...
    <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5'>
      ...
      <input value={email} onChange={(e)=>setEmail(e.target.value)} className='bg-transparent outline-none text-white' type="email" placeholder='Email id' />
...
{isOtpSubmitted && isEmailSent &&
    ...
    <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5'>
      ...
      <input value={newPassword} onChange={(e)=>setNewPassword(e.target.value)} className='bg-transparent outline-none text-white' type="password" placeholder='password' />
